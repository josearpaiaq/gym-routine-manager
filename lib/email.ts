  ...
  await resend.emails.send({
    ...
    html: `
      <div style="font-family:sans-serif;max-width:400px;margin:0 auto;padding:32px">
        ...
        <p style="color:#6b7280;margin:0 0 24px">Ingresa este código en la app para acceder a tu cuenta.</p>
        <div style="background:#1f2937;border-radius:12px;padding:24px;text-align:center">
          <span style="font-size:36px;font-weight:700;letter-spacing:12px;color:#fff">${code}</span>
        </div>
        <p style="color:#9ca3af;font-size:12px;margin:16px 0 0">Expira en 10 minutos. Si no solicitaste este código, ignora este email.</p>
      </div>
    `,
  });
}

export async function sendDeleteAccountOtpEmail(email: string, code: string): Promise<void> {
  await resend.emails.send({
    from: "Gym Manager <onboarding@gym-manager.lat>",
    to: email,
    ...
    html: `
      <div style="font-family:sans-serif;max-width:400px;margin:0 auto;padding:32px">
        <h2 style="margin:0 0 8px">Eliminar cuenta</h2>
        <p style="color:#6b7280;margin:0 0 24px">Ingresa este código para confirmar que deseas eliminar tu cuenta. Esta acción no se puede deshacer.</p>
        <div style="background:#1f2937;border-radius:12px;padding:24px;text-align:center">
          <span style="font-size:36px;font-weight:700;letter-spacing:12px;color:#fff">${code}</span>
        </div>
        <p style="color:#9ca3af;font-size:12px;margin:16px 0 0">Expira en 10 minutos. Si no solicitaste esto, ignora este email.</p>
      </div>
    `,
  });
}
